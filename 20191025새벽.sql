...
--
 ...
 create or replace procedure proc_rentaldelayed
    ...
    begin
     for delayedlist in (select * from rental where rent_enddate<sysdate and rent_status='대여중' ) loop -- 책 반납 안하는 사람들.
    update member set deadline_rent_stop=nvl(deadline_rent_stop,sysdate)+1,book_loanable='대출정지' where mem_id=delayedlist.mem_id;
    ...
    update member set book_loanable='대출가능' where mem_id=deadlinecheck.mem_id;
     ...
     update member set deadline_rent_stop=sysdate-10 ,book_loanable='대출정지' where mem_id='nmj';
     select * from book;
     select * from rental;
 ...
 fetch ho into countvalues;  -- 
  exit when ho%notfound;
   ...
   -- 새로운 프로시저 하나로 묶을수 있을듯.
    ...
    delete from rental where book_num=countvalues.book_num and rent_status='예약중';
   --1번인사람 판단하기
...
RENT_STATUS    )
        ...
        --아래 위험
      --
--      insert into reservation    1번에 할당
        end if;
...
--     
    ...
    select * from reservation;
     ...
     --책 반납하자마자 
     --예약  취소 <-- delete 
     select * from (select rownum rnum, b.book_num, book_author, book_pub_house, book_name, book_pub_date, book_ISBN, book_apdx_status, book_rsrv_status, book_ctgr_num, book_input_date, decode(rent_enddate,null,'대여중','대여가능') rent,rent_status , rentalmem_id  from (select * from book where book_num = 100001) b left outer join (select book_num, nvl(to_char(rent_enddate),null) rent_enddate,rent_status , rental.mem_id rentalmem_id from rental where rent_startdate in (select max(rent_startdate) from rental group by book_num) and rent_num in (select max(rent_num) from rental group by book_num) ) r on (b.book_num = r.book_num)) 
/
...
select * from book;
--대출여부, 연장여부,예약가능여부
delete from rental where  rent_status = '예약중';
