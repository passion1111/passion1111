...
insert into loc(loc_name, loc_contents, loc_filename, loc_regname, loc_writer) values('자유의 여신상', '명불허전 관광명소. 자유의 나라 미국을 의미하는 상징물로 리버티 섬에 위풍당당하게 서있다. 미국 독립 100주년을 기념해 프랑스에서 선물로 주었으며, 작가는 프레데릭 바르톨디, 설계는 에펠 탑을 설계한 귀스타브 에펠이 맡았다. 오른손에는 횃불을, 왼손에는 독립선언서를 들고 있는 이 여인의 내부에는 엘리베이터가, 왕관에는 전망대가 설치되어 있으며, 받침대 안은 박물관으로 운영되고 있다. 내부로 들어갈 수 있는 인원이 시간당 제한되고 있기 때문에 사전예약은 필수이다. 최소 2~3개월 전에 인터넷 예약을 통해서만 내부로 들어갈 수 있다. 그 크기가 어마어마해 섬 안에서는 사진을 제대로 찍을 수 없으니 페리를 타고 올 때 미리 찍어두자.', '자유의 여신상.jpg', '뉴욕', '조규민');
insert into loc(loc_name, loc_contents, loc_filename, loc_regname, loc_writer) values('타임스퀘어', '뉴욕 여행의 필수 스팟이자 뉴욕의 랜드마크! 뉴욕 타임스 본사가 이곳으로 이전하면서 타임 스퀘어라는 이름을 갖게 됐다. 삼각형 모양의 광장에 화려한 네온사인과 광고가 펼쳐지며 여행자를 매혹시킨다.세계에서 가장 비싼 광고비가 드는 곳으로도 유명하며 현재 삼성, LG 광고가 나오고 있다. 여행자가 가장 많이 몰리는 곳이기 때문에 비지터 센터와 각종 기념품 숍 등 다양한 볼거리가 가득하다. 광장과 연결되어있는 브로드웨이를 따라 위치한 극장에서 공연을 관람할 수도 있어 24시간 화려한 뉴욕이 느껴지는 곳이다.', '타임스퀘어.jpg', '뉴욕', '조규민');
insert into loc(loc_name, loc_contents, loc_filename, loc_regname, loc_writer) values('브로드웨이', '런던 웨스트엔드와 함께 세계 뮤지컬을 이끌고 있는 브로드웨이! 세계적 수준의 공연들이 화려하게 펼쳐지는 곳이다. 타임스 스퀘어 주변으로 50개에 달하는 극장이 있으며 대형 뮤지컬뿐만 아니라 소극장 공연도 있다. 대부분 공연의 티켓이 빨리 매진되기 때문에 최대한 이른 시간에 가서 예매해야 한다. 뮤지컬 극장에 가서 하는 방법과 타임스 스퀘어에 있는 티켓부스를 이용하는 방법이 있으며 인터넷 예매도 가능하니 참고하자.', '브로드웨이.jpg', '뉴욕', '조규민');
insert into loc(loc_name, loc_contents, loc_filename, loc_regname, loc_writer) values('뉴욕 증권거래소', '월 스트리트의 메인 스팟이라고 볼 수 있는 뉴욕 증권거래소. 수많은 영화에도 등장했었던 이곳은 세계 최대 규모의 증권거래소로 위풍당당한 건물과 커다란 성조기가 특징이다. 내부 출입은 금지되어 있지만 뉴욕을 대표하는 장소인 만큼 외부 기념 사진을 찍고 가자.', '뉴욕 증권거래소.jpg', '뉴욕', '조규민');
