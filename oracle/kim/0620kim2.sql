...
select p_code,         --p code를 찾고  decode라는 메소드를 써서 product테이블에 pcode가 100이면 뉴욕 101 달러스 등등 이렇게 쓰기위한것.
        decode (p_code,100,'new work',
                            101,'DALLAS',
                            102,'BOSTON',
                            103,'CHICAGO',
                                '지역을 몰라요')
                                "Location of inventory"
                                from product where p_code<103;
...
SELECT product_id,
       ...
       WHERE product_id < 1775 order by warehouse_id;
