...
--주석
select *from emp2;
select empno,deptno,emp_type,name from emp2;
select *from emp2 where emp_type='계약직'
--order by name;  오름차순
order by name desc;  --내림차순

select *from tab;  --테이블 전체를 보는것

desc emp2;--대체하는 방법은 그냥 테이블 열면 됨

select sysdate from dual;

create table userlist(id varchar2(10) constraint id_pk primary key, --constraint가 제약조건 id_pk안쓰면 기본키가없는 제약조건을만들어주는것
name varchar2(10) not null
);
...
insert into gift select 1,'그래',3,4
