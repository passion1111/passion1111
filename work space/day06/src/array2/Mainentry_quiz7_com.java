...
public class Mainentry_quiz7_com {
	...
	for(int k=0;k<arr.length;k++) {
		...
		//  System.out.println(rank_name[j][0]);
	  ...
	  for(;k<arr.length;k++) {
		  ...
		  System.out.println("****"+name[k]+"님의 성적표입니다.****");
			...
			System.out.println(name[k]+"님의 학점은 ");
			System.out.println("국어 :" + grade3[k] + "\t영어: " +grade[k] + "\t전산: " + grade2[k]);
			System.out.printf("총점: " +sum[k][0] + "\t평균: %.2f ", ave[k][0] ,"\t 평균학점 : %s",grade4[k] );
			System.out.println("평균학점:"+ave[k][0]);
			System.out.println("석차"+rank_name[k][0]);
