...
//			insert stock 테이블 동기화부분(준모)
			pstmt = conn.prepareStatement("ALTER TABLE STOCK ADD "+ bvrname +" NUMBER DEFAULT(10)");
			pstmt.executeUpdate();
...
//			동기화부분 끝
	...
	//------------------------------------------------ commit/rollback 선택
	public void confirmModification() throws SQLException, InterruptedException {
		while (true) {
			String sel = sc.next();
