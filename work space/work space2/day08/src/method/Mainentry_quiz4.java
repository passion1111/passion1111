...
public class Mainentry_quiz4 {
	...
	public static int[]rank(int[]rank,int[]sum,int[]sum_compare){
		for(int k=0;k<rank.length;k++) {
		rank[k]=1;
		}
		for(int j=0;j<rank.length;j++) {
		 for(int k=0;k<rank.length;k++) {
			 if( sum[j]<sum_compare[k]) {
				 rank[j]+=1;
			 }
		 }
		}
...
//		int[][] arr=new int[totalnumber][3];	//국영수 저장하는곳
		...
		String[]name=name(totalnumber);////각자 입력하게되면 문제 메소드를 이렇게 나누면한번에 이름,과목으로 저장하기가 힘이
//		 												들어 각자  저장하는것이 가독성이 좋음.
		
		//total(arr,totalnumber);  //총점
		int[] sum=new int[totalnumber]; //배열대입을 위해 변수생성
		
		int[] sum1=total(sum,arr,totalnumber);//총점을 sum1에 저장.  
		int[]sum_compare=sum1.clone();             //석차순위저장을위해 배열 복사
		average(totalnumber,sum1);				//평균
		double ave[]=average(totalnumber,sum1);	//	평균
		grade(ave,totalnumber,arr,sum);			//등급 .
		String[][]grade_ave=grade(ave,totalnumber,arr,sum).clone();	//	등급값을 저장할 그릇.
		dd2(sum1,sum_compare);//석차 비교를 위한 알고리즘.
		int[]sum_compare1=dd2(sum1,sum_compare).clone();	//비교한 값을 저장
		System.out.println(sum_compare1[0]);
		 rank=rank(rank,sum1,sum_compare);		     //순위를 저장.
		output(totalnumber,name,sum1,rank,arr,grade_ave,ave);  //~번쨰가아닌 ~님의 라고 고쳐야함.
}
}
