...
public class QABoardreplyDAO {
	...
	public int getreplynumlv(int traRepWriteRep, int traRepNumRef) {
		...
		try {
			...
			if (rs.next())
				tradeBoarReplyNum = rs.getInt(1) + 1;
		} catch (Exception e) {
	...
	public ArrayList<QABoardreplyVO> getreplylist(int tradeboardnum) {
	...
	// 아래부터는 보여줄 게시내용만.
	public ArrayList<QABoardreplyVO> getreplylist2(int tradeboardnum, int startreplynum, int endreplynum) {
